...
const createApiClient = () => {
    return {
        ...
        administratorLogin: async (username, password) => {
            ...
            return await axios.get(`http://132.72.116.72:3039/admin/administratorLogin/${username}/${password}`).then((res) => res);
            },
        changeAgentParams:(agent,latency,variance)=>{
            console.log("in api changeAgentParams- agent: ", agent, " latency: ",latency," variance: ",variance);
            return axios.post(`http://132.72.116.72:3039/agents/updateAgentByAgentType/${agent}/${latency}/${variance}`).then((res) => res);
        },
        getParamValue:(paramName)=>{
            console.log("in api getParamValue- paramName: ", paramName);

            return axios.post(`http://132.72.116.72:3039/params/getParamValue/${paramName}`).then((res) => res);
        },
        getAllParamsValue:()=>{
            console.log("in api getAllParamsValue");
            return axios.post(`http://132.72.116.72:3039/params/getAllParamsValue`).then((res) => res);
        },
        changeParamValue:(paramName,value)=>{
            console.log("in api changeParamValue- paramName: ", paramName," value: ",value);
            return axios.post(`http://132.72.116.72:3039/params/updateParamValue/${paramName}/${value}`).then((res) => res);
        },
        registerPlayer:(mail,age,gender,deviceUid,version)=>{
            ...
            return axios.post(`http://132.72.116.72:3039/users/addUser/${mail}/${age}/${gender}/${deviceUid}/${version}`).then((res) => res);
        },
        sendAnswers:(deviceUUID,agentType,qDict)=>{
            ...
            return axios.post(`http://132.72.116.72:3039/dictAns/addAnswers/${deviceUUID}/${agentType}/${ans1}/${ans2}/${ans3}/${ans4}/${ans5}/${ans6}/${ans7}`).then((res) => res);
        },
        sendQuestionnaireAnswers:(qDict,agentType,deviceUid)=>{
            ...
            return axios.post(`http://132.72.116.72:3039/feedback/addFeedback/${deviceUid}/${agentType}`).then((res) => res);
        ...
        sendFeedBack:(deviceUUID, feedBack)=>{
            ...
            return axios.post(`http://132.72.116.72:3039/feedback/addFeedback/${deviceUUID}/${feedBack}`).then((res) => res);
        },
        sendPressTimeStamp:(deviceUUID,playerTimeStampArr, agentTimeStampArr, experimentInfo)=>{
            ...
            return axios.post(`http://132.72.116.72:3039/actions/addAction/${deviceUUID}/${playerTimeStampArr}/${agentTimeStampArr}/${experimentInfo}`).then((res) => res);
        },
    }
